...
function [thresh,testStat] = hsicTestBoot(X,Y,alpha,params, fileNameSuf);
...
threshFileName = strcat('hsicTest_',fileNameSuf);
...
%  disp(strcat('Generating new threshold: ',threshFileName))
  ...
  if doSaveThresh
    save(threshFileName,'thresh','HSICarr', "params", "testStat", "indices", "X", "Y", "alpha");
  end
