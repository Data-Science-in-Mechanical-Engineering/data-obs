...
function [testStat,thresh,params] = mmdTestBoot(X,Y,alpha,params,fileNameSuf)
...
threshFileName = strcat('mmdTest_',fileNameSuf);
...
%  disp(strcat('Generating new threshold: ',threshFileName))
  ...
  indices = zeros(params.shuff, 2*m);
  for whichSh=1:params.shuff
    ...
    indices(whichSh, :) = indShuff;
  end

  MMDarr = sort(MMDarr);
  thresh = MMDarr(round((1-alpha)*params.shuff));

  save(threshFileName,'thresh','MMDarr', 'params', 'testStat', 'indices', 'X', 'Y', 'Kz', 'alpha');
